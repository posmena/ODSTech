...
mongoexport -d odstech -c live_forthillhome --csv -f 'id','title','link','price','shipping','description','condition','image_link','category','quantity','availability','google_product_type','brand','mpn' -o files/hostedfeeds/forthillhome/froogle.csv | mail -s "Forthill Froogle Exported" tech@odst.co.uk < logs/forthill.log
