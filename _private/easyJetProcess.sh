...
for region in 'Algarve' 'Amsterdam' 'Barcelona' 'Corfu' 'Crete' 'Cyprus' 'Egypt' 'Fuerteventura' 'Halkidiki' 'Ibiza' 'Kos' 'Lanzarote' 'Lombardy' 'Madeira' 'Madrid' 'Majorca' 'Malta' 'Menorca' 'Morocco' 'Mykonos' 'Paris' 'Prague' 'Rhodes' 'Rome' 'Santorini' 'Tenerife' 'Turkey' 'Venice' 'Zante'; do
        mongodump --db odstech --collection packageschecksum --query '{"region":"'$region'"}' > region.log
        ...
        echo 'http://www.odst.co.uk/feeds/easyjet.zip?region='$region >> region.log
...
mongodump --db odstech --collection packageschecksum --query '{"region":"Costa Blanca"}' > region.log
region='Costa_Blanca'
somevar="files/hostedfeeds/easyjet/region"$region".csv"
mongorestore -d odstech -c tempfeed --drop dump/odstech/packageschecksum.bson
echo 'http://www.odst.co.uk/feeds/easyjet.zip?region='$region >> region.log
...
mongodump --db odstech --collection packageschecksum --query '{"region":"Costa Brava"}' > region.log
region='Costa_Brava'
somevar="files/hostedfeeds/easyjet/region"$region".csv"
mongorestore -d odstech -c tempfeed --drop dump/odstech/packageschecksum.bson
echo 'http://www.odst.co.uk/feeds/easyjet.zip?region='$region >> region.log
...
mongodump --db odstech --collection packageschecksum --query '{"region":"Costa Del Sol"}' > region.log
...
mongodump --db odstech --collection easyjet_scrape > region.log
region='Costa_Dorada'
somevar="files/hostedfeeds/easyjet/region"$region".csv"
...
mongodump --db odstech --collection packageschecksum --query '{"region":"Gran Canaria"}' > region.log
region='Gran_Canaria'
somevar="files/hostedfeeds/easyjet/region"$region".csv"
mongorestore -d odstech -c tempfeed --drop dump/odstech/packageschecksum.bson
echo 'http://www.odst.co.uk/feeds/easyjet.zip?region='$region >> region.log
...
mongodump --db odstech --collection packageschecksum --query '{"region":"Tel Aviv"}' > region.log
region='Tel_Aviv'
somevar="files/hostedfeeds/easyjet/region"$region".csv"
mongorestore -d odstech -c tempfeed --drop dump/odstech/packageschecksum.bson
echo 'http://www.odst.co.uk/feeds/easyjet.zip?region='$region >> region.log
...
echo 'http://www.odst.co.uk/feeds/easyjet.zip?propertylist=true' > region.log
mongoexport -d odstech -c easyjet_properties --csv -f propertyid,name,resort,resortid,region,country,rating,hoteltype,airportcode,image1url,image2url,image3url,description,address,price,url -o files/hostedfeeds/easyjet/propertiesfull.csv | mail -s 'Easyjet Properties Exported' tech@odst.co.uk < region.log
./zipFiles.sh
