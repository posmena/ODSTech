...
class timthumb {
	...
	protected function processImageAndWriteToCache($localImage){
		...
		if( empty($new_height) ) { $new_height = $new_width; } ;
		if( empty($new_width) ) { $new_width = $new_height; } 
		...
		$a = $this->newSize($width, $height, $new_width, 0);
		
	    if( $a[1] > $new_height )
			{
			$a = $this->newSize($width, $height, 0, $new_height);		
			}
	...
	protected function newSize($width, $height, $new_width, $new_height) {
  // generate new w/h if not provided
