...
(function ($) {
	...
	$.fn.liveTile.defaults = {
		...
		initDelay: 2000,                        // how long to wait before the initial animation
		...
		alwaysTrigger: true,                   // used by flip-list to decide if all tiles are triggered every time
	...
	var pubMethods = {
		init: function (options) {
			...
			return $(this).each(function () {
				...
				tdata.initDelay = (!stgs.ignoreDataAttributes && typeof ($this.data("initdelay")) != "undefined") ? $this.data("initdelay") : stgs.initDelay;
				...
				$this.slide = function () {
					...
					if (canTransition && tdata.useHwAccel) {
						...
						if (tdata.direction == "vertical") {
							css.top = (slidingUp && tdata.stops.length == 1) ? "0px" : stop;
						} else {
							css.left = (slidingUp && tdata.stops.length == 1) ? "0px" : stop;
						}
						$firstContainer.css(css).data("tile", data);
						if (tdata.stack == true) {
							if (tdata.direction == "vertical") {
								css.top = (slidingUp && tdata.stops.length == 1) ? -metric + 'px' : offset;
							} else {
								css.left = (slidingUp && tdata.stops.length == 1) ? -metric + 'px' : offset;
							}
						...
						var uCss = (tdata.direction == "vertical") ?
									{ top: (slidingUp && tdata.stops.length == 1) ? "0px" : stop} :
									{ left: (slidingUp && tdata.stops.length == 1) ? "0px" : stop };
						var dCss = (tdata.direction == "vertical") ?
									{ top: (slidingUp && tdata.stops.length == 1) ? -metric + 'px' : offset} :
									{ left: (slidingUp && tdata.stops.length == 1) ? -metric + 'px' : offset };
							...
							if (trigger) {
								window.setTimeout(function () {
									if (!swapFrontSource) {
										...
										$this.flipListItem(true, $t, $front, $back, image, isBground);
									}
								}, triggerSpeed);
							}
						} else {
							if (trigger) {
								window.setTimeout(function () {
									if (!swapBackSource) {
										...
										$this.flipListItem(false, $t, $back, $front, image, isBground);
									}
								}, triggerSpeed);
							}
						}
